...
def run_landaus(title, folder_path, d_mip_variables, data_charge_inputs):
	...
	rplt.hist(
		...
		fill = True,
		alpha=0.5,
		label='Cluster Charge Distribution from Data',
	)
	rplt.hist(
		...
		fill = True,
		alpha=0.5,
		label='Strip Charge Distribution from Data',
	)
	ax.set_ylim([0,0.15])
	ax.set_xlabel('Charge [e]')
	ax.set_ylabel('N')
	...
	########################################################################################################################
	### Reweighted MC input vs charge from gain.
	########################################################################################################################
	fig = plt.figure()
	ax = fig.add_subplot(1, 1, 1)
	rplt.hist(
		...
		fill = True,
		alpha=0.5,
		label='Weighted Signal Charge Deposition Simulation',
	)
	rplt.hist(
		charge_read_hist, 
		histtype='step',
		...
		fill = True,
		alpha=0.5,
		label='Weighted Charge Deposition Simulation from Gain',
	)
	ax.set_ylim([0,0.1])
	ax.set_xlabel('Charge [e]')
	ax.set_ylabel('N')
	...
	rplt.hist(
		...
		fill = True,
		alpha=0.5,
		label='Strip Charge Deposition Simulation',
	)
	rplt.hist(
		data_charge_inputs['hist_stripCharge_normed'], 
		histtype='step',
		...
		fill = True,
		alpha=0.5,
		label='Strip Charge Deposition Data',
	)
	...
	rplt.hist(
		...
		fill = True,
		alpha=0.5,
		label='Normalised Data Charge Read',
	)
	rplt.hist(
		data_charge_inputs['hist_stripCharge_normed'], 
		histtype='step',
		...
		fill = True,
		alpha=0.5,
		label='Normalised Data Charge From Single Cluster Read',
	)
	rplt.hist(
		charge_deposited_hist, 
		histtype='step',
		...
		fill = True,
		alpha=0.5,
		label='Normalised Simulated Charge Deposition',
	)
	rplt.hist(
		charge_read_hist, 
		histtype='step',
		...
		fill = True,
		alpha=0.5,
		label='Normalised Simulated Charge Read',
	)

	ax.set_ylim([0,0.05])
	ax.set_xlabel('Charge [e]')
	ax.set_ylabel('N')
	fig.suptitle('Data Normed', fontsize=14, fontweight='bold')
	plt.title(title, loc='right')
	leg = plt.legend(loc='best')
	leg.draw_frame(False)	
